...
class PhotoWCT(nn.Module):
    ...
    def transform(self, cont_img, styl_img, cont_seg, styl_seg):
        ...
        # print(cont_seg)
        csF4 = self.__feature_wct(cF4, sF4, cont_seg, styl_seg, True)
        ...
        csF3 = self.__feature_wct(cF3, sF3, cont_seg, styl_seg, False)
        ...
        csF2 = self.__feature_wct(cF2, sF2, cont_seg, styl_seg, False)
        Im2 = self.d2(csF2, cpool_idx, cpool)

        cF1 = self.e1(Im2)
        sF1 = sF1.data.squeeze(0)
        cF1 = cF1.data.squeeze(0)
        csF1 = self.__feature_wct(cF1, sF1, cont_seg, styl_seg, False)
        Im1 = self.d1(csF1)
        return Im1
    ...
    def __compute_label_info(self, cont_seg, styl_seg):
        ...
        for l in self.label_set:
            ...
            #cont_seg = np.asarray(Image.fromarray(cont_seg).convert('L'))
           
            #styl_seg = np.asarray(Image.fromarray(styl_seg).convert('L'))
    ...
    def __feature_wct(self, cont_feat, styl_feat, cont_seg, styl_seg, noise=False):
        ...
        if cont_seg.size == False or styl_seg.size == False:
            target_feature = self.__wct_core(cont_feat_view, styl_feat_view, noise)
        else:
            target_feature = cont_feat.view(cont_c, -1).clone()
            
            t_cont_seg = np.asarray(Image.fromarray(cont_seg, mode='L').resize((cont_w, cont_h), Image.NEAREST))
           
            t_styl_seg = np.asarray(Image.fromarray(styl_seg, mode='L').resize((styl_w, styl_h), Image.NEAREST))
            ...
            for l in self.label_set:
                ...
                # print(len(cont_indi))
                # print(len(styl_indi))
                tmp_target_feature = self.__wct_core(cFFG, sFFG, noise)
    ...
    def __wct_core(self, cont_feat, styl_feat, noise):
        ...
        contentConv = torch.mm(cont_feat, cont_feat.t()).div(cFSize[1]) + iden
        ...
        styleConv = torch.mm(styl_feat, styl_feat.t()).div(sFSize[1])
        ...
        # noise svd
        if noise == True:
            noiseFeature1 = torch.randn(k_s,k_s).float()

            n_u, n_e, n_v = torch.svd(noiseFeature1, some=False)
            #n_v = noiseFeature1
            noiseFeature = torch.mm(torch.mm(torch.mm(torch.mm(s_v[:, 0:k_s], torch.diag(s_d)), n_v.cuda()),(s_v[:, 0:k_s].t())), whiten_cF)
            originalFeature = torch.mm(torch.mm(torch.mm(s_v[:, 0:k_s], torch.diag(s_d)), (s_v[:, 0:k_s].t())), whiten_cF)

            ratio_n = 1
            targetFeature = ratio_n * noiseFeature + (1 - ratio_n) * originalFeature
