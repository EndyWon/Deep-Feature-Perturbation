...
class AvatarNet(object):
    ...
    def transfer_styles(self,
        ...
        inputs_image_features = losses.extract_image_features(
            inputs, self.network_name,reuse=True)
...
def zca_normalization(features):
    ...
    # noise
    sh = tf.shape(u)
    noise = tf.random_normal(shape=(sh[1],sh[2]),mean=0,stddev=1,dtype=tf.float32)
    ...
    sqrt_inv_s_effective=tf.reshape(sqrt_inv_s_effective,shape=(sh[1],sh[2]))
    unbiased_features=tf.reshape(unbiased_features,shape=(-1,sh[2]))

    # colorization functions
    colorization_kernel_n = tf.matmul(tf.matmul(u_n, sqrt_s_effective), u, transpose_b=True)
    colorization_kernel = tf.matmul(tf.matmul(u, sqrt_s_effective), u, transpose_b=True)
    ...
    ratio_n = 0.5
    colorization_kernel = ratio_n * colorization_kernel_n + (1 - ratio_n) * colorization_kernel
