...
cmd:option('-contentSize', 0, 'New (minimum) size for the content image, keeping the original size if set to 0')
cmd:option('-styleSize', 0, 'New (minimum) size for the style image, keeping the original size if set to 0')
...
function feature_wct(contentFeature, styleFeature, lambda, add_noise)
   ...
   -- content feature whitening
    ...
    if opt.gpu >= 0 then
        whiten_contentFeature = (c_v[{{},{1,k_c}}]:cuda()) * torch.diag(c_d:cuda()) * (c_v[{{},{1,k_c}}]:t():cuda()) *contentFeature1
        if add_noise == true then   
            perturbedFeature = (s_v[{{},{1,k_s}}]:cuda()) * (torch.diag(s_d1:cuda())) * (orthogonalNoise:cuda()) * (s_v[{{},{1,k_s}}]:t():cuda()) * whiten_contentFeature
            originalFeature = (s_v[{{},{1,k_s}}]:cuda()) * (torch.diag(s_d1:cuda())) * (s_v[{{},{1,k_s}}]:t():cuda()) * whiten_contentFeature
            targetFeature = lambda * perturbedFeature + (1.0 - lambda) * originalFeature  
        else
        ...
        whiten_contentFeature = c_v[{{},{1,k_c}}] * torch.diag(c_d) * (c_v[{{},{1,k_c}}]:t()) * contentFeature1
        if add_noise == true then   
            perturbedFeature = (s_v[{{},{1,k_s}}]) * (torch.diag(s_d1)) * orthogonalNoise * (s_v[{{},{1,k_s}}]:t()) * whiten_contentFeature
            originalFeature = (s_v[{{},{1,k_s}}]) * (torch.diag(s_d1)) * (s_v[{{},{1,k_s}}]:t()) * whiten_contentFeature
            targetFeature = lambda  * perturbedFeature + (1.0 - lambda ) * originalFeature   
        else
...
local function styleTransfer(content, style, iteration)
    ...
    if opt.swap5 ~= 0 then   
        ...
        csF5 = feature_wct(cF5, sF5, opt.lambda, true)
    end
    ...
    local csF4 = feature_wct(cF4, sF4, opt.lambda, false)
    ...
    local csF3 = feature_wct(cF3, sF3, opt.lambda, false)
    ...
    local csF2 = feature_wct(cF2, sF2, opt.lambda, false)
    ...
    local csF1 = feature_wct(cF1, sF1, opt.lambda, false)
...
for i=1,numContent do
    ...
    for j=1,numStyle do
        ...
        if opt.synthesis == 0 then
            ...
            local savePath = paths.concat(opt.outputDir, contentName .. '_diverse_stylized_by_' .. styleName .. '_alpha_' ..opt.alpha*100 .. '_lambda_' ..opt.lambda*100 .. '.' .. opt.saveExt)
...
print(string.format('time: %f seconds',t))
